...
export const AuthScreen = ({ onAuthSuccess }: AuthScreenProps) => {
  ...
  const STAGE_CREDENTIALS = {
    group: { username: "rmc2025_group", password: "rmc2025@group" },
    playoffs: { username: "rmc2025_playoffs", password: "rmc2025@playoffs" },
    semifinals: { username: "rmc2025_semifinals", password: "rmc2025@semifinals" },
    finals: { username: "rmc2025_finals", password: "rmc2025@finals" },
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const isOrganizer =
        username === ORGANIZER_CREDENTIALS.username &&
        password === ORGANIZER_CREDENTIALS.password;

      if (isOrganizer) {
        sessionStorage.setItem(
          "mpl_auth",
          JSON.stringify({ username, role: "organizer" })
        );
        toast({ title: "Welcome!", description: "Logged in as organizer." });
        onAuthSuccess(true);
        setIsLoading(false);
      ...
      for (const [stage, creds] of Object.entries(STAGE_CREDENTIALS)) {
        if (username === creds.username && password === creds.password) {
          ...
          sessionStorage.setItem(
            "mpl_auth",
            JSON.stringify({ username, role: "player", stage: stageFound })
          );
          toast({
            title: "Access Granted",
            description: `Access granted to ${stageFound} questions.`,
          });
          onAuthSuccess(false, stageFound);
          setIsLoading(false);
          return;
    ...
    <div className="min-h-screen bg-gradient-stadium flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
          className="absolute -top-40 -left-40 w-96 h-96 bg-blue-500/20 rounded-full blur-3xl"
          style={{ boxShadow: "0 0 100px 50px rgba(59, 130, 246, 0.3)" }}
        />
        <motion.div
          animate={{ scale: [1, 1.3, 1], rotate: [0, -90, 0], opacity: [0.15, 0.25, 0.15] }}
          transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
          className="absolute -bottom-40 -right-40 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl"
          style={{ boxShadow: "0 0 100px 50px rgba(147, 51, 234, 0.3)" }}
        />
      </div>
        ...
        <Card className="p-8 bg-card/95 backdrop-blur-xl shadow-2xl border-2 border-primary/20">
          ...
          >
            ...
            >
              ...
              <motion.img
                src="/assets/logos/intellalpha.png"
                alt="Intell Alpha Logo"
                className="w-10 h-10 rounded-md object-cover shadow-md"
                whileHover={{ scale: 1.05 }}
              />
            </motion.div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            >
              <Button
                ...
                style={{
                  boxShadow:
                    "0 0 30px rgba(37, 99, 235, 0.4), 0 4px 6px rgba(0, 0, 0, 0.1)",
                }}
              >
                {isLoading ? (
                  <motion.div
