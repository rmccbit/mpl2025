...
export const GameScreen = ({
  ...
  const handleAnswer = (result: {
    ...
    if (!selectedBatter || !selectedBowler) {
      toast({
        ...
        description: "Please select both a batter and a bowler before proceeding.",
        variant: "destructive",
      });
      return;
    }
    ...
    setGameState((prev) => {
      const batterName = selectedBatter;
      const bowlerName = selectedBowler;
      ...
      if (result.isExtra) {
        ...
        return {
          ...prev,
          runs: prev.runs + added,
          extras: prev.extras + added,
          ballDetails: [...prev.ballDetails, ballDetail],
        };
      }
      ...
      if (result.batterCorrect) {
        ...
        newWickets += 1;
        setPopup({ type: "wicket", message: "Bowler got it right!" });
      } else {
        setPopup({ type: "dot", message: "Both missed it!" });
      }

      const BALLS_PER_INNINGS = 10;

      if (prev.innings === 1 && newBalls >= BALLS_PER_INNINGS) {
        setPopup({ type: "extra", message: `End of innings 1. Target: ${newRuns + 1} runs.` });
        return {
          ...prev,
          teamAScore:
            prev.battingTeam === "A"
              ? { runs: newRuns, wickets: newWickets, overs: newOvers }
              : prev.teamAScore ?? { runs: newRuns, wickets: newWickets, overs: newOvers },
          innings: 2,
      ...
      if (prev.innings === 2) {
        ...
        if (target && newRuns >= target) {
          const winnerName = prev.battingTeam === "A" ? teamAName : teamBName;
          setPopup({ type: "winner", message: `${winnerName} won by chasing the target!` });
          return {
            ...prev,
            runs: newRuns,
            wickets: newWickets,
        ...
        if (newBalls >= BALLS_PER_INNINGS) {
          const firstRuns = prev.teamAScore?.runs ?? 0;
          const winnerName =
            newRuns > firstRuns
              ? prev.battingTeam === "A"
                ? teamAName
          ...
          setPopup({
            type: "winner",
            message:
              winnerName === "Tie" ? "The match is a tie." : `${winnerName} won the match!`,
          });
          return {
            ...prev,
            runs: newRuns,
  ...
  const battingTeamPlayers =
    gameState.battingTeam === "A" ? teamAPlayers : teamBPlayers;
  const bowlingTeamPlayers =
    gameState.battingTeam === "A" ? teamBPlayers : teamAPlayers;

  // Save game data when over
  useEffect(() => {
    if (gameState.gameOver && gameState.winner && gameState.ballDetails.length > 0) {
      const gameData: GameData = {
        teamA: { name: teamAName, players: teamAPlayers, score: gameState.teamAScore },
      ...
      api.saveGame(gameData).then(() =>
        toast({ title: "Game Saved!", description: "All data recorded." })
      );
    }
  }, [gameState]);
  ...
  return (
    <div className="min-h-screen bg-gradient-stadium p-4 animate-fade-in">
      {popup && (
        <GamePopup type={popup.type} value={popup.value} message={popup.message} onClose={() => setPopup(null)} />
      )}

      <div className="flex justify-end mb-2 gap-2">
        <button className="px-4 py-2 bg-primary text-white rounded" onClick={onNavigateToDashboard}>
          📊 Dashboard
        </button>
        <button className="px-4 py-2 bg-secondary text-white rounded" onClick={onNewGame}>
          New Game
        </button>
      </div>

      <Scoreboard teamAName={teamAName} teamBName={teamBName} gameState={gameState} />

      <div className="grid lg:grid-cols-[300px_1fr_300px] gap-4 mt-4">
        {/* Batting Team */}
        <div className="bg-white/10 p-3 rounded-xl shadow">
          <h2 className="text-xl font-semibold text-center mb-2">{gameState.battingTeam === "A" ? teamAName : teamBName} (Batting)</h2>
          <ul className="space-y-1">
            {battingTeamPlayers.map((p) => (
              <li
                key={p}
                onClick={() => setSelectedBatter(p)}
                className={`cursor-pointer px-3 py-1 rounded-lg text-center ${
                  selectedBatter === p ? "bg-green-500 text-white" : "hover:bg-green-200"
                }`}
              >
                {p}
              </li>
        ...
        {/* Bowling Team */}
        <div className="bg-white/10 p-3 rounded-xl shadow">
          <h2 className="text-xl font-semibold text-center mb-2">{gameState.battingTeam === "A" ? teamBName : teamAName} (Bowling)</h2>
          <ul className="space-y-1">
            {bowlingTeamPlayers.map((p) => (
              <li
                ...
                className={`cursor-pointer px-3 py-1 rounded-lg text-center ${
                  selectedBowler === p ? "bg-blue-500 text-white" : "hover:bg-blue-200"
                }`}
              >
                {p}
              </li>
